...
def main(args):
    ...
    for idx, dp in tqdm(enumerate(dataset)):
        ...
        nice_print(f"Before response: {clean_result['before_response']}")
        nice_print(f"After response: {clean_result['after_response']}")
        ...
        print(f"Before ASR: {round(sum([r['clean_result']['before_success'] for r in results]) / len(results), 2)}, {clean_result['before_success']}")
        print(f"After ASR: {round(sum([r['clean_result']['after_success'] for r in results]) / len(results), 2)}, {clean_result['after_success']}")
        print(f"Before Utility: {round(sum([r['clean_result']['before_utility'] for r in results]) / len(results), 2)}, {clean_result['before_utility']}")
        print(f"After Utility: {round(sum([r['clean_result']['after_utility'] for r in results]) / len(results), 2)}, {clean_result['after_utility']}")
        print(f"No Defense Time: {round(sum([r['clean_result']['no_defense_time'] for r in results]) / len(results), 2)}, {clean_result['no_defense_time']}")
        print(f"Defense Time: {round(sum([r['clean_result']['defense_time'] for r in results]) / len(results), 2)}, {clean_result['defense_time']}")
        print(f"Precision: {round(sum([r['clean_result']['precision'] for r in results]) / len(results), 2)}, {clean_result['precision']}")
        print(f"Recall: {round(sum([r['clean_result']['recall'] for r in results]) / len(results), 2)}, {clean_result['recall']}")
        print(f"F1: {round(sum([r['clean_result']['f1'] for r in results]) / len(results), 2)}, {clean_result['f1']}")
        ...
        nice_print(f"Before response: {heuristic_result['before_response']}")
        nice_print(f"After response: {heuristic_result['after_response']}")
        ...
        print(f"Before ASR: {round(sum([r['heuristic_result']['before_success'] for r in results]) / len(results), 2)}, {heuristic_result['before_success']}")
        print(f"After ASR: {round(sum([r['heuristic_result']['after_success'] for r in results]) / len(results), 2)}, {heuristic_result['after_success']}")
        print(f"Before Utility: {round(sum([r['heuristic_result']['before_utility'] for r in results]) / len(results), 2)}, {heuristic_result['before_utility']}")
        print(f"After Utility: {round(sum([r['heuristic_result']['after_utility'] for r in results]) / len(results), 2)}, {heuristic_result['after_utility']}")
        print(f"No Defense Time: {round(sum([r['heuristic_result']['no_defense_time'] for r in results]) / len(results), 2)}, {heuristic_result['no_defense_time']}")
        print(f"Defense Time: {round(sum([r['heuristic_result']['defense_time'] for r in results]) / len(results), 2)}, {heuristic_result['defense_time']}")
        print(f"Precision: {round(sum([r['heuristic_result']['precision'] for r in results]) / len(results), 2)}, {heuristic_result['precision']}")
        print(f"Recall: {round(sum([r['heuristic_result']['recall'] for r in results]) / len(results), 2)}, {heuristic_result['recall']}")
        print(f"F1: {round(sum([r['heuristic_result']['f1'] for r in results]) / len(results), 2)}, {heuristic_result['f1']}")
        ...
        if gcg_input_prompt is not None:
            ...
            if "target" in dp:
                nice_print(f"Attack Target: {dp['target']}")
            nice_print(f"Before response: {gcg_result['before_response']}")
            nice_print(f"After response: {gcg_result['after_response']}")
            ...
            print(f"Before ASR: {round(sum([r['gcg_result']['before_success'] for r in results]) / len(results), 2)}, {gcg_result['before_success']}")
            print(f"After ASR: {round(sum([r['gcg_result']['after_success'] for r in results]) / len(results), 2)}, {gcg_result['after_success']}")
            print(f"Before Utility: {round(sum([r['gcg_result']['before_utility'] for r in results]) / len(results), 2)}, {gcg_result['before_utility']}")
            print(f"After Utility: {round(sum([r['gcg_result']['after_utility'] for r in results]) / len(results), 2)}, {gcg_result['after_utility']}")
            print(f"No Defense Time: {round(sum([r['gcg_result']['no_defense_time'] for r in results]) / len(results), 2)}, {gcg_result['no_defense_time']}")
            print(f"Defense Time: {round(sum([r['gcg_result']['defense_time'] for r in results]) / len(results), 2)}, {gcg_result['defense_time']}")
            print(f"Precision: {round(sum([r['gcg_result']['precision'] for r in results]) / len(results), 2)}, {gcg_result['precision']}")
            print(f"Recall: {round(sum([r['gcg_result']['recall'] for r in results]) / len(results), 2)}, {gcg_result['recall']}")
            print(f"F1: {round(sum([r['gcg_result']['f1'] for r in results]) / len(results), 2)}, {gcg_result['f1']}")
        print()
